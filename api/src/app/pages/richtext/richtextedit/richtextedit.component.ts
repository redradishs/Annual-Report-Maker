...
@Component({
  selector: 'app-richtextedit',
  standalone: true,
  imports: [NavbarComponent, NgFor, CommonModule],
  templateUrl: './richtextedit.component.html',
  styleUrl: './richtextedit.component.css'
})
  ...
  this.textInput.nativeElement.innerHTML = html;
    ...
    fetchDocument(): void {
      if (this.userId && this.postId) {
        this.api.getDocument(this.postId).subscribe(
          response => {
    ...
    saveContent(): void {
      if (this.textInput && this.textInput.nativeElement) {
        ...
        if (this.userId !== null && this.postId !== null) {
          this.api.getUsername(this.userId).subscribe(
            response => {
              let username = response.trim(); 
              username = username.replace(/^"(.*)"$/, '$1');
              const body = { content, edited_by: username };
    
              this.api.editDocuments(this.postId!, body).subscribe(
                response => {
