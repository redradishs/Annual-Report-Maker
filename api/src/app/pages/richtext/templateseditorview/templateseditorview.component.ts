...
@Component({
  selector: 'app-templateseditorview',
  standalone: true,
  imports: [NavbarComponent, FormsModule, CommonModule, PaginationComponent],
  templateUrl: './templateseditorview.component.html',
  styleUrl: './templateseditorview.component.css'
})
export class TemplateseditorviewComponent {
  ...
  fetchContent(documentId: string) {
    if (this.userId !== null) {
      this.api.fetchTemplate(documentId).subscribe(
        (resp: any) => {
          console.log(resp);
  ...
  editContent() {
    this.router.navigate(['/template/edit', this.postId]);
  }
