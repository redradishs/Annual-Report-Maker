...
@Component({
  selector: 'app-templateseditorblank',
  standalone: true,
  imports: [NavbarComponent, NgFor, CommonModule],
  templateUrl: './templateseditorblank.component.html',
  styleUrl: './templateseditorblank.component.css'
})
export class TemplateseditorblankComponent {
  ...
  constructor(private http: HttpClient, private authService: AuthService, private router: Router, private chatbotloader: ChatbotLoaderService, private dialog: MatDialog, private renderer: Renderer2, private api: ApiService) {
  ...
  ngOnInit(): void {
    ...
    this.authService.getCurrentUser().subscribe(user => {
      if (user) {
        ...
        this.uploadUrl = this.api.getUploadUrl(this.userId!);
        this.fetchUrl = this.api.getFetchUrl(this.userId!);
        this.fetchImages();
      } else {
...
//end table
  ...
  // image starts here
  fetchImages(): void {
    this.api.fetchImages(this.fetchUrl).subscribe(
      (response) => {
  ...
  onFileSelected(event: any): void {
    ...
    if (file) {
      ...
      this.api.uploadImage(this.uploadUrl, formData).subscribe(
        (response) => {
          if (response.success) {
            this.fetchImages();
          } else {
  ...
  //       // Move the cursor to the end of the inserted header
  //       const newRange = document.createRange();
  //       newRange.setStartAfter(header);
  //       newRange.setEndAfter(header);
  //       selection.removeAllRanges();
...
// insert header stops here 
  ...
  initializer(): void {
   ...
   if (this.imageUpload) {
    ...
    // jQuery initialization for form submission
    // $(document).ready(() => {
    //   $('#saveForm').on('submit', () => {
    //     $('#textContent').val($('#text-input').html());
    //   });
    // });
  }
