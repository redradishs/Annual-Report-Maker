...
@Component({
  selector: 'app-richtextcollabedit',
  standalone: true,
  imports: [NavbarComponent, NgFor, CommonModule],
  templateUrl: './richtextcollabedit.component.html',
  styleUrl: './richtextcollabedit.component.css'
})
export class RichtextcollabeditComponent {
  ...
  saveContent(): void {
    if (this.textInput && this.textInput.nativeElement) {
      ...
      if (this.userId !== null && this.postId !== null) {
        this.apiService.getUsername(this.userId).subscribe(
          response => {
            let username = response.trim(); 
            username = username.replace(/^"(.*)"$/, '$1');
            this.apiService.editDocument(this.postId!, content, username).subscribe(
              response => {
                console.log('Content saved:', response);
                this.router.navigate(['document/view/output', this.postId]); 
              },
              error => {
        ...
        console.error('User ID or Post ID is null');
      }
    } else {
...
handleDocumentClick(event: MouseEvent): void {
  ...
  fetchImages(): void {
    this.apiService.fetchElements(this.userId!).subscribe(
      (response) => {
        console.log('Fetch Images Response:', response); 
        if (response.data && Array.isArray(response.data)) {
          this.images = response.data.map((item: any) => {
            const imageUrl = this.apiService.constructImageUrl(item.file_path);
            return imageUrl;
          });
        } else {
  ...
  onFileSelected(event: any): void {
    ...
    if (file) {
      ...
      this.apiService.uploadElements(this.userId!, formData).subscribe(
        (response) => {
          if (response.success) {
            this.fetchImages();
          } else {
